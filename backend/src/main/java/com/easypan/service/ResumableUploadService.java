 ...
 * 
 * 提供文件上传进度管理和断点续传功能
 * 使用 Redis 记录上传进度，支持客户端断线重连后继续上传
 * 
 * Redis Key 设计：
 * - upload:progress:{userId}:{fileMd5}:completed - Set 类型，存储已完成的分片索引
 * - upload:progress:{userId}:{fileMd5}:total - String 类型，存储总分片数
 * 
 * 需求：2.3.2
 */
@Service
